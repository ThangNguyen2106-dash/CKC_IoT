...
inline void CKC_PnP<Transport>::SaveWiFi(String newSSID, String newPASS)
{
    ...
    for (int i = WiFi_MAX - 1; i > 0; i--)
    {
        ...
        prefs.putString(("ssid" + String(i)).c_str(), saved_ssid[i]);
        prefs.putString(("pass" + String(i)).c_str(), saved_pass[i]);
    }
...
inline void CKC_PnP<Transport>::loadWiFi()
{
    ...
    {
        saved_ssid[i] = prefs.getString(("ssid" + String(i)).c_str(), "");
        saved_pass[i] = prefs.getString(("pass" + String(i)).c_str(), "");
    }
    prefs.end();
}
...
inline void CKC_PnP<Transport>::CKC_state_Connect_STA()
{
    ...
    for (int j = 0; j < WiFi_MAX; j++)
    {
        ...
        if (WiFi.status() == WL_CONNECTED)
        {
            strcpy(_sta_ssid, saved_ssid[j].c_str());
            strcpy(_sta_pass, saved_pass[j].c_str());
            STA();
            return;
        }
...
inline void CKC_PnP<Transport>::CKC_state_Connect_AP()
{
    ...
    webServer.on("/", [this]()
                 { configMode = true; lastUserconfig = millis(); this->webServer.send(200, "text/html", this->htmlPage()); });
    webServer.on("/connect", HTTP_POST, [this]()
                 { this->handleSave(); });
    webServer.on("/scan", [this]()
                 { configMode = true; lastUserconfig = millis(); this->handleScan(); this->webServer.send(200, "text/html", this->htmlPage()); });
    webServer.begin();
...
inline void CKC_PnP<Transport>::AP()
{
    ...
    if (configMode)
    {
        ...
        {
            configMode = false;
            CKC_LOG_DEBUG("WIFI", "USER TIMEOUT -> AUTO RECONNECT");
        }
        else
        {
            return;
